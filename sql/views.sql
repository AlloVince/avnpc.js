...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW view_eva_blog_posts
    AS SELECT
       ...
       `slug` AS `slug`,
       `contentStorage` AS `contentStorage`,
       `contentRemoteUrl` AS `contentRemoteUrl`,
       `contentRemoteHash` AS `contentRemoteHash`,
       FROM_UNIXTIME(IF(contentSynchronizedAt > 0, contentSynchronizedAt, NULL)) AS contentSynchronizedAt,
